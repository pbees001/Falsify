...
(function ($) {
  	jQuery.fn.itis = function (options){
		...
		var is_bp = ($.inArray('bp',is)>=0)?true:false, bpo, bpl,
			...
			is_and = ($.inArray('and',is)>=0)?true:false,
        ...
        for (i in $ul) {
          ...
          //if (i != '1') {
            $ul[i].each(function() {
              ...
              if ($a.length>0 && !$a.next(tc).length) $a.after('<span class="'+tc+'"></span>');
            });
          //};
        };
      }
      ...
      $elements.each(function(itm){
        ...
        if ( cook ) {
          if ( !($.cookie(options.cookieName) == null) ){
            aid = $.cookie(options.cookieName) == ''?[]:$.cookie(options.cookieName).split(' ');
        ...
        if (type == 'a') {
          if ( cook && aid.length > 1 ) {
            ...
            $ae = $(id).find(options.titleInSelector);
            if ($ae.length) {
              $ae.click();
              if (options.scrollToTab){
                $('html, body').animate({
          ...
          $ae = $wrap.find(options.titleOutSelector+'[href="'+id+'"]');
          if ($ae.length) $ae.click();
        } else if (type == 'd') {
          var $e = $(id), $ae = $e;
...
;
/*
  Prepare structure inputs labels & label 0.4 (06.17.20)
  Author: Pavel Tkach
*/
(function($){
  jQuery.fn.labelsPrepare = function(options){
    ...
    $(this).each(function(i) {
      ...
      if ( ( type == 'checkbox' && options.checkbox ) ||  (type == 'radio' && options.radio ) || (type == 'text' && options.text  )) {
...
(function ($, Drupal) {
  ...
  Drupal.behaviors.ai = {
    ...
    // checkbox to select
        function radioToSelect (selForm,selWrapCheck) {
          $(selForm).each(function(i1) {
            var  $form = $(this), $sel = $('<select id="chs'+i1.toString()+'"></select>').insertBefore($form);
            $form.find(selWrapCheck).each(function(i2){
              var $l = $(this).find('label'), $i = $(this).find('input'),
              str = '<option value="'+i2.toString()+'" '+($i.attr('checked')?'selected':'')+'>'+$l.text()+'</option>';
              $(str).appendTo($sel);
            });
            $sel.select({
              allLabel: false,
              ...
              onSelectChanged: function() {
                var $chb = $(this).closest('.wrap-sb').next('form').find(selWrapCheck + ' input'),val = parseInt($(this).attr('data-value'));
                $chb.eq(val)[0].checked  = true;
                $chb.eq(val).change();
              },
            });
          });
     ...
     // list to select
        function listToSelect (selList) {
          $(selList).each(function(i1) {
            var  $ul = $(this), $sel = $('<select id="lss'+i1.toString()+'"></select>').insertBefore($ul);
        ...
        function menuWidth(self,col) {
          var l1 = $('.b-menu-secondary > .container')[0].offsetLeft;
          var max = 0, $items = $(self).find('> li > a'),
          ww = window.innerWidth, wh = window.innerHeight,
          $e2, $e3, l2 = 0, l3 = 0, w2 = 0,
          $bl = $(self).closest('.b-menu-secondary');
          $bl.removeClass('no-fixed ws-normal');
          $(self).css('width','auto');
          if ( $(self).hasClass('level3') ) {
            $e3 = $(self).closest('li');
            $e2 = $e3.closest('li');
            l3 = $e3[0].offsetLeft;
          } else if ( $(self).hasClass('level2') ) {
            $e2 = $(self).closest('li');
          };
          l2 = $e2[0].offsetLeft;
          w2 = $e2[0].offsetWidth;
          if ( (self.offsetHeight + 50) > wh) {
            $bl.addClass('no-fixed ws-normal');
          }  else $bl.removeClass('no-fixed ws-normal');
          max = $items[0].offsetWidth*col+20*(col-1);
          if ((l1+l2+l3+max) > ww)  {
            if (max > ww) max = ww - 40;
            $(self).css('left',((ww-max)/2-l1-l2+10)+'px');
          };
          $bl.addClass('ws-normal');
          $(self).css('width',max+'px');
        };
        function eventColorC() {
          $('.calendar-calendar td .inner',context).each(function (j) {
            var colors = [],$clr;
            $(this).find('[data-colors]').each(function () {
              var c = $(this).attr('data-colors').split(',');
              for (var i = 0; i < c.length; i++) {
                if ( c[i] != '' &&  colors.indexOf(c[i]) == -1 ) colors.push(c[i]);
              }
            });
            if (colors.length>0) {
              $clr = $('<div class="colors"></div>').appendTo($(this));
              for (var i = 0; i < colors.length; i++) {
                $('<span class="event-color '+colors[i]+'"></span>').appendTo($clr);
              };
            };
          });
        };
        function eventColorL() {
          $('.b-events-list .content-body .teaser > a',context).each(function (j) {
              var colors = [],$clr;
              var c = $(this).attr('data-colors').split(',');
              for (var i = 0; i < c.length; i++) {
                if ( c[i] != '' &&  colors.indexOf(c[i]) == -1 ) colors.push(c[i]);
              ...
              if (colors.length>0) {
                if($(this).find('.four').length) {
                  $clr = $(this).find('.four').html('');
                }  else $clr = $('<div class="four"></div>').appendTo($(this));
                for (var i = 0; i < colors.length; i++) {
                  $('<span class="event-color '+colors[i]+'"></span>').appendTo($clr);
                };
              };
      ...
      $(document, context).ready(function() {
        if ( $('body').hasClass('_rfix') ) { return; }
          else $('body').addClass('_rfix');
        $(window).itis({is:['ie','ios','mt','load']});
      // main-menu
        var $main_menu = $('.menu--main',context),$sf;
       if ($main_menu.length) {
         $main_menu.once('menu').menumobile({
          mobile: 1023,
            levelSelectors: {
              '1':['{}>ul','>li','>a,>span'],
              '2':['{1}>ul','>li','>a,>span'],
          ...
          if (!$('body').hasClass('no-menu')) {
            if ( !$('body').hasClass('sticky-secondary-menu') ) {
              $('#primary_menu',context).sticky().on('sticky-start', function() {
              ...
              $('.b-menu-secondary').each(function() {
                var element = $(this);
          ...
          $sf = $('.search-form.mobile').wrap('<li class="search" />');
          $sf.parent().appendTo($main_menu.find('.level1'));
       };

       if ($('.b-menu-secondary nav',context).length) {
          $('.b-menu-secondary nav',context).menumobile({
            mobile: 1023,
            onlyOneOpen: true,
            levelSelectors: {
              '1':['{}>ul','>li','>a,>span'],
            ...
            htmlToggler: '<span class="toggler"></span>',
            classBody: 'secondary-menu-opened',
            classLinkOff: 'linkoff',
          });
       };
      // footer toggler
        $('.b-footer-links h6').once().on('click',function () {
          var $bl = $(this).closest('.b-footer-links');
          if ($bl.hasClass('open')) {
            $bl.removeClass('open');
          } else {
            $bl.addClass('open');
      ...
      // select
        $('#page select',context).not('#state_select').not('[name="items_per_page"]').not(function () {
          if ($(this).closest('.b-text').length) return true;
          return false;
        }).select({
          allLabel: false,
        ...
        $('.b-custom-webform.b-text select,.migrated select',context).select({
          allLabel: false,
          classCopy: 'required error',
          height: 290,
          eventContainer: '#page',
        ...
        // checkbox
        $('.webform-submission-form input[type="checkbox"],.webform-submission-form input[type="radio"]',context).labelsPrepare();
        //  labels
        $('.b-custom-webform.b-text input[type="text"]',context).labelsPrepare({text:true});
      // ol-list
        $('.b-text ol').each(function(){
          var  type = $(this).attr('type');
          if(type === 'A') $(this).attr('type','ua');
          if(type === 'a') $(this).attr('type','la');
          if(type === 'I') $(this).attr('type','ur');
          if(type === 'i') $(this).attr('type','lr');
        });
      //match-height
        //$('.b-hero-home .label .content').matchHeight({byRow: true});
        $('.b-3column .header').matchHeight({
          byRow: true,
        });
        $('.b-carousel-home .body').matchHeight({byRow: false});
        $('.b-carousel-home h3').matchHeight({byRow: false});
        //$('.b-carousel .second').matchHeight({byRow: false});
        $('.card2 h5').matchHeight({byRow: true});
        $('.card2 p').matchHeight({byRow: false});
        $('.card3 h3,.card5 h3').matchHeight({byRow: true});
        $('.card3 p,.card5 p').matchHeight({byRow: true});
        $('.card6 h3').matchHeight({byRow: true});

      //slick slider
        $('.slick.one').each(function() {
          var ap = $(this).hasClass('rotate');
          if( $(this).find('.item').length>1 )  {
            ...
            $('.b-carousel-home .slick.one').on('beforeChange', function(event, slick, currentSlide, nextSlide){
        ...
        // history slider
        $('.slick.history-one').slick({
          slidesToShow: 1,
          slidesToScroll: 1,
          arrows: false,
          //fade: true,
          asNavFor: '.slick.history-two',
          //autoplay: true
        });
        $('.slick.history-two').slick({
          infinite: false,
          slidesToShow: 12,
          slidesToScroll: 1,
          asNavFor: '.slick.history-one',
          dots: false,
          //centerMode: true,
          focusOnSelect: true,
          responsive: [
      ...
      //tabs
        $('.content-tabs .titles > li',context).once('content-tabs').toggler({
          type: 'tabs',
          wrapperSelector: '.content-tabs',
          titleOutSelector: '.titles > li',
          titleInSelector: '> .title',
        ...
        // all standard tabs
        $('.tabs .titles > li',context).once('tabs').toggler({
          type: 'tabs',
      ...
      // accordion
        $('.accordion .atab',context).once('accordion').toggler({
          type: 'accordion',
          itemSelector: '.atab',
          bodySelector: '.abody',
          activeTabByURL: true,
        ...
        $('.atab iframe').wrap('<div class="iframe" />');
        // filters
        if($('body.pg-resources',context).length == 1 || $('body.path-resources-listing',context).length == 1) {
          var id_act_tab = '';
          //read cookie
          if ( !($.cookie('sidebar_active_tab') == null) ){
            id_act_tab = $.cookie('sidebar_active_tab');
            //console.log('sidebar_active_tab= ',id_act_tab);
            //history.pushState(null, '', '?id='+id_act_tab);
            history.pushState(null, document.title, window.location.origin+window.location.pathname);
          };
          $('#layout-sidebar-first .tab',context).once('filter').toggler ({
            type: 'accordion',
            wrapperSelector: '.region-sidebar-first',
            bodySelector: '.acc-tab',
            titleInSelector: 'h2',
            toggleSelf: true,
            //activeTabByURL: true,
        ...
        $('.b-accordion .more').click(function() {
          var $bl = $(this).closest('.b-accordion'),
          $at = $bl.find('.atab'), $ab = $bl.find('.abody');
          if (!$at.hasClass('open')) {
            $at.addClass('open');
            $(this).text('Hide All');
        ...
        // iframe
            ...
            // stop play
            $(document).on('click','.b-carousel-home .slick-arrow',function(e) {
              var $btn = $(this);
              var wrapper = $btn.closest('.slick');//.find('.slick-current');
              stopAllYouTubeVideos();
              //if ( wrapper.length ) videoStop(wrapper);
            });
            // start play
            $(document).on('click','.b-video-card .first,.b-video-card .icon-youtube-play',function(e) {
              if ( !$(e.target).hasClass('close') ) {
                e.preventDefault();
                var $btn = $(this);
                var $wrapper = $btn.find('.wrapper-video').addClass('overlay');
                $('#page').addClass('overlay');
                $('body').addClass('no-scroll');
                videoPlay($wrapper);
                $wrapper.find('.close').click(function() {
                  videoStop($wrapper)
                  $(this).closest('.wrapper-video').removeClass('overlay');
        ...
        // checkbox to select
            //radioToSelect('.b-2col-tabs form','.form-type-radio');
        // list to select
        listToSelect('.b-2col-tabs.faq .first .titles');
        // add pdf icon to links
        function addIcon($elm) {
          var href = $elm.attr('href'), ft = !$elm.text()?false:true;
          if ( href && href.length > 4) {
            if  ( href.slice(-4) == '.pdf' && ft) {
              $elm.append('<span class="ico-pdf"></span>');
              if (drupalSettings.ai_stat == true) {
                if  ( href.slice(0,1) == '/' ) href = 'https://www.usccb.org' + href;
                $elm.append('<a data-dialog-options="{&apos;width&apos;:800}" data-dialog-type="modal" href="/ai/stat/popup?path='+href+'" class="use-ajax ai-atlas-header-request-support"><span class="ico-chart"></span></a>');
              }
            }
        ...
        $('.b-2col-tabs a, .b-text a,.b-prayer a').not('.btn').each(function() {
          addIcon($(this));
        });
        $('.b-resorce-detail a').not('.btn').each(function() {
          addIcon($(this));
        });
        // migrate style
        //$('.wr-block.b-text').addClass('migrated');
        $('.wr-block.migrated').each(function(){
          $(this).find('p').each(function() {
            var  $fch = $(this).children(':first-child');
            if (this.innerHTML == '&nbsp;'|| this.innerHTML == '') $(this).addClass('blank');
            if ( $fch && ($fch.is('sup') || $fch.hasClass('superscript')) && $(this).html()[0] == '<') $(this).addClass('footnote');
          });
          $(this).find('blockquote').each(function() {
            var $bq = $(this), $ch = $bq.children('blockquote'), ht = $(this).html(), $d = $('<div></div') ;
            if ( $ch.length>0 ) {
              $d.insertAfter($bq).html(ht).addClass('blq');
              $bq.remove();
            }
          });
          $(this).find('blockquote').each(function() {
            var $ch = $(this).children();
            if ( $ch && $ch.length == 1 && ($ch.hasClass('blank') || !$ch.text()) ) {
              $(this).addClass('blank');
            };
          });
          ...
          $(this).find('img').each(function() {
            if ( $(this).hasClass('img-right') || $(this).hasClass('img-left') ) {
              $(this).wrap('<div class="wimage"></div>')
              var $wr = $(this).parent();
              if ( $(this).hasClass('img-right') ) $wr.addClass('right');
              if ( $(this).hasClass('img-left') ) $wr.addClass('left');
            };
          });
          $(this).find('table td').each(function() {
            var txt = $(this).text();
            if(this.innerHTML == '&nbsp;'|| this.innerHTML == '') $(this).addClass('blank_mb');
            if($(this).attr('colspan')) $(this).parent().addClass('title_mb');
            if($(this).children('h2').length) {
              $(this).addClass('h2_mb');
              if($(this).children('h2')[0].innerHTML == '&nbsp;'|| $(this).children('h2')[0].innerHTML == '') $(this).children('h2').addClass('blank_mb');
            };
            if ( txt.length == 1 && txt.charCodeAt(0) == 160 ) $(this).addClass('blank_mb');
          });
          $(this).find('table tr.title_mb').each(function() {
            var txt = $(this).text();
            if ( !txt.trim().length && !$(this).find('img').length ) $(this).addClass('title_blank_mb');
          });
          $(this).addClass('show');
        });
        ...
        ;
        $('.wr-block.capitalize-h2 h2').each(function(){
          $(this).text(capitalize($(this).text(),true));
        });
        // external link
        var  pat = ["/","http://store.usccb.org","https://store.usccb.org","http://usccb.org","http://www.usccb.org","https://www.usccb.org","https://usccb.org","https://bible.usccb.org",
        "https://prayers-usccb.pantheonsite.io","https://dev-usccb.pantheonsite.io","https://dev-usccb-readings.pantheonsite.io",
        "https://readings.usccb.allianceinteractive.org/","http://cms.usccb.org",
        "http://www.facebook.com","http://twitter.com","http://www.youtube.com","https://www.linkedin.com","https://www.instagram.com",
        "mailto:","tel:","?","#"], pattern, ancor;
        ...
        ancor = new RegExp("^#");
        $('.region-content  .wr-block a').each(function(){
          var href = $(this).attr('href'), nw = ($(this).attr('target') == "_blank")?true:false,
              tmp, fl = false;
          if (typeof href === 'string') {
            if (/^javascript/.exec(href) !== null ) {
              tmp = href.split(',');
              href = tmp[2].substr(0,tmp[2].length-3);
              $(this).attr('href',href);
            };
            if ( pattern.exec(href) === null ) {
              $(this).addClass('icon-link-ext');
              $(this).on('click', function (event) {
                event.preventDefault();

                Swal.fire({
                  width: 375,
                  titleText: "You're leaving USCCB.org",
                  text:"By accepting this message, you will be leaving the USCCB website. By providing the link USCCB assumes no responsibility for, nor does it necessarily endorse, the website, its content, or sponsoring organizations.",
                  reverseButtons: true,
        ...
        //parallax image
        $('.wr-block.b-parallax-image').each(function() {
          var $pr = $(this).prev(), $ne = $(this).next();
          if ($pr.length && ($pr.hasClass('bg-container') || $pr.hasClass('b-title-page'))) $pr.addClass('ext-bg-page ext-padding-bottom-m');
          if ($ne.length && ($ne.hasClass('bg-container') || $pr.hasClass('b-title-page'))) $ne.addClass('ext-bg-page ext-padding-top-m');
        });
        if($('.parallax-window',context).length) {
          $('.parallax-window',context).parallax({bleed:50}).resize();
        };
        // smooth scrolling
        // var $page = $('html, body');
        ...
        // resources pager
        if ( $('.view-exposed-filters').length ) {
          if ( $('.b-pager.full').length ) {
            $('select[name="items_per_page"]').select({
              allLabel: false,
              hideselect: true,
              selSelect: 'select[name="items_per_page"]',
              classCopy: 'required error',
              height: 200,
              onSelectChanged: function() {
                $('.view-exposed-filters .form-submit').click();
              },
            });
            $('.b-pager.full .pager__item--previous').wrap("<li class='section-1'><ul></ul></li>");
            var $s4 =  $('.b-pager.full .pager__item--next').wrap("<li class='section-4'><ul></ul></li>").parent().parent();
            $('.b-pager.full .pager__item').not('.pager__item--previous,.pager__item--next').wrapAll("<li class='section-2'><ul></ul></li>");
            var $s3 = $("<li class='section-3'></li>" ).insertBefore($s4);
            var $sb = $('.view-exposed-filters .selectBox');
            // var $sb = $('#block-exposedformsearchpage-1 .selectBox');
            $s3.prepend($sb).prepend('<span>VIEW ROWS</span>');
          }  else {
            $('select[name="items_per_page"]').css({'display':'none'});
          };
        };
        // autocomplete off
        $('form').on('focus','.bef-datepicker',function(){
          $(this).attr('autocomplete', 'off');
        });
        // search
        $('#header .icon-search').toggler({
          type: 'toggler',
          addToggleElements:[
            {'search':'#header'},
          ],
          preventDefault: true,
          togglerContainer: {inner:'#search-form',outer:'body'},
        });
        // very long secondary menu
        $('.columns-2.expanded > ul').each(function() {
            menuWidth(this,2);
         });
        $('.columns-3.ex363panded > ul').each(function() {
            menuWidth(this,3);
         });
        // subsribed message
        if ( !$('body').hasClass('IE') ) {
          // $('.b-overlay',context).each(function() {
          //   var $ov = $(this), ovm = ($.cookie('ov-message') == null),
          //   $subm = $(this).find('.form-submit');
          //   if ( !ovm ) $ov.removeClass('show').addClass('hide');
          //   $ov.on('click','.close',function() {
          //     var exp = $(this).hasClass('one-year')?365:1;
          //     $.cookie('ov-message', true, { expires: exp });
          //     $ov.removeClass('show').addClass('hide');
          //   });
          //   $subm.data('ai_subscribe',{
          //     'fok': function () {
          //       $.cookie('ov-message', true, { expires: 365 });
          //       $ov.removeClass('show').addClass('hide');
          //     },
          //   });
          // });
        ...
        // unsubscribe form
        var  $usf = $('.b-subscribe.unsubscribe');
        if ( $usf && $usf.length ) {
          var $all = $usf.find('#all'), $usi = $usf.find('.usitem');
        ...
        $('.pg-contact-us .form-submit').click(function (e) {
          if ( !($('#edit-topic')[0].selectedIndex  > 0 || $('#edit-office')[0].selectedIndex > 0) ) {
            e.preventDefault();
            Swal.fire({
              text: 'Select a Topic or an Office/Committee, please!',
              customClass: { popup: 'validation'}
            });
            return  false;
          }
          return true;
        ...
        // show books panel
        $('.b-books-bottom-panel,.b-books-side-panel',context).each(function() {
          var $ov = $(this), ovm, mes ="books-panel";
          ovm = ($.cookie(mes) == null);
          if ( !ovm ) $ov.addClass('hide');
          $ov.on('click','.icon-cancel',function() {
            var exp = 7; //days
            $.cookie(mes, true, { expires: exp });
            $ov.addClass('hide');
          });;
        });
        // $('.f-contact').click(function () {
        //   $(this).find('input,select').each(function () {
        //     console.log(this.id, this.tabIndex);
        //   });
        // })
        // $('.f-contact').on('blur',function (e) {
        //   //console.log(e.target);
        // });
        // $('.form-item-topic .selectBox').attr("tabindex", 0);
        // $('.form-item-office .selectBox').attr("tabindex", 0);
        // $('#edit-privacy-policy').attr("tabindex", 0);
        // $('.form-submit').attr("tabindex", 0);
      //  set events color
        eventColorC();
        eventColorL();
      ...
      $(document).ajaxComplete(function (){
        var $select = $('#page select',context).not('#state_select').not('[name="items_per_page"]').not(function () {
          if ($(this).closest('.b-text').length) return true;
          return false;
        });
        if (context == document) {
          ...
          $('.b-custom-webform.b-text select',context).select({
            allLabel: false,
            classCopy: 'required error',
            height: 290,
            eventContainer: '#page',
          ...
          if(typeof addthis !== 'undefined') { 
            ...
            $('a.share').click(function(e) {
              e.preventDefault();
              $('#edit-help .at-share-btn').each(function() {$(this).click()})
            }); 
          }           
        };
        // checkbox to select
        // if ($('.b-2col-tabs .first .wrap-sb').length <  1){
        //   radioToSelect('.b-2col-tabs form','.form-type-radio');
        // };
      //  set events color
        eventColorC();
      ...
      // slick slider only for mobile
      $(window).on('load resize orientationchange', function() {
        // $('.carousel-mobile, .b-teasers.images .view-content').each(function(){
        //   var $carousel = $(this);
        //   if ($(window).width() > 520) {
        //     if ($carousel.hasClass('slick-initialized')) {
        //       $carousel.slick('unslick');
        //       $('.carousel-mobile .body').matchHeight('remove');
        //     }
        //   }
        //   else{
        //     if (!$carousel.hasClass('slick-initialized')) {
        ...
        //         mobileFirst: true,
        //       });
        //       $('.carousel-mobile .body').matchHeight({
        //         byRow: false,
        //       });
        //     }
        //   }
        ...
        if ($(window).width() > 1023) {
          ...
          $('.columns-3.expanded > ul').each(function() {
              menuWidth(this,3);
          });
        };
      });
...
(function ($, Drupal, drupalSettings) {
  ...
  Drupal.eu_cookie_compliance.createPopup = function (html, closed) {
    ...
    $('body').addClass('eu-cookie-compliance-banner');
  ...
  Drupal.eu_cookie_compliance.acceptAction = function () {
    ...
    if (!agreedEnabled) {
      ...
      nextStatus = 2;
      $('body').removeClass('eu-cookie-compliance-banner');
    }
